import plotly.express as px
import numpy as np


# function to plot interactive plotly chart
def interactive_plot(df):
    fig = px.line()
    for i in df.columns[1:]:
        fig.add_scatter(x=df['Date'], y=df[i], name=i)
    fig.update_layout(
        width=450,
        margin=dict(l=20, r=20, t=50, b=20),
        legend=dict(
            orientation='h',
            yanchor='bottom',
            y=1.02,
            xanchor='right',
            x=1
        )
    )
    return fig

# function to normalize the price based on the initial price
def normalize(df_2):
    df=df_2.copy()
    for i in df.columns[1:]:   # skip Date column
        df[i] = df[i] / df[i].iloc[0] 
    return df 

# function to calculate daily returns
def daily_return(df_2):
    df_daily_return=df_2.copy()
    for i in df_daily_return.columns[1:]:
        for j in range(1, len(df_daily_return)):
            df_daily_return.loc[j, i] = (df_daily_return.loc[j, i] - df_daily_return.loc[j-1, i]) / df_daily_return.loc[j-1, i]

    return df_daily_return

import numpy as np

# function to calculate beta and alpha
def calculate_beta(stocks_daily_return, stock):
    # Market return (annualized mean of SP500 daily returns)
    rm = stocks_daily_return['SP500'].mean() * 252
    # Linear regression: stock return vs market return
    b, a = np.polyfit(stocks_daily_return['SP500'], stocks_daily_return[stock], 1)
    return b, a








    
